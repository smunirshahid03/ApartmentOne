@extends('Dashboard.Layouts.master_dashboard')
@section('content')
    <div class="income-reports-sec-page basic-reports-box properties-page admin-users-page">
        <div class="row">
            <div class="col-md-12">


                <div class="two-things-align">
                    <div class="box">
                        <h6>User Details</h6>
                        <p>All users will be show here</p>
                    </div>
                    <div class="box">
                        <ul class="nav nav-tabs" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link active" data-toggle="tab" href="#tabs-1" role="tab">All</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" data-toggle="tab" href="#tabs-2" role="tab">Tenants</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" data-toggle="tab" href="#tabs-3" role="tab">land Lords</a>
                            </li>
                        </ul><!-- Tab panes -->
                    </div>
                </div>


                <div class="tab-content">

                    <div class="tab-pane p-3 active" id="tabs-1" role="tabpanel" aria-expanded="true">
                        <div class="reports-table">
                            <table>
                                <tr>
                                    <th>Tenant Name</th>
                                    <th>Joined</th>
                                    <th>Role</th>
                                    <th>Action</th>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                            </table>
                        </div>
                    </div>

                    <div class="tab-pane p-3" id="tabs-2" role="tabpanel" aria-expanded="false">

                        <div class="reports-table">
                            <table>
                                <tr>
                                    <th>Tenant Name</th>
                                    <th>Joined</th>
                                    <th>Role</th>
                                    <th>Action</th>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                            </table>
                        </div>
                    </div>

                    <div class="tab-pane p-3" id="tabs-3" role="tabpanel" aria-expanded="false">

                        <div class="reports-table">
                            <table>
                                <tr>
                                    <th>Tenant Name</th>
                                    <th>Joined</th>
                                    <th>Role</th>
                                    <th>Action</th>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                                <tr>
                                    <td>Robert Strom</td>
                                    <td>January 2024</td>
                                    <td>Tenant</td>
                                    <td>
                                        <a href="#">Edit</a>
                                        <a href="#">Delete</a>
                                    </td>
                                </tr>
                            </table>
                        </div>

                    </div>



                    </div>


                </div>



            </div>
        </div>
    </div>


    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.0.8/popper.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/4.0.0-beta/js/bootstrap.min.js"></script>
@endsection
